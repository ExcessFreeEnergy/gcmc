...
def cli(argv=None):
    parser = argparse.ArgumentParser(
        description="Run GCMC simulations for short-ranged Gaussian truncated potentials."
    )
    parser.add_argument(
        "-in", "--input_folder",
    ...
    if args.engine == "v2":
        try:
            ...
            print(f"[gcmc] Warning: v2 engine encountered an issue ({e}), falling back to v1 engine...", file=sys.stderr)
