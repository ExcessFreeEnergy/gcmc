...
class OneComponentPlasmaPotential(Potential):
    def __init__(self, epsilon, q, kappa_inv):
        ...
        self.prefactor = (self.q * const.elementary_charge) ** 2 / (
            4 * const.pi * const.epsilon_0 * 1e-10 * self.epsilon
        )
...
class HardSphereCoulombPotential(Potential):
    ...
    def calculate(self, r):
        return np.where(
            r < self.diameter, very_large_number, self.prefactor * self.q1 * self.q2 * erfc(r / self.kappa_inv) / r
        )
...
def initialize_potentials(config):
    ...
    for pair, params in config["potential_pairs"].items():
        ...
        if pair_type == "LJ":
            potential_dict[pair] = LennardJonesPotential(
                epsilon=params["epsilon"], sigma=params["sigma"], rc=params["rc"]
            )
        elif pair_type == "WCA":
            ...
            potential_dict[pair] = OneComponentPlasmaPotential(
                epsilon=params["epsilon"], q=params["q"], kappa_inv=params["kappa_inv"]
            )
        elif pair_type == "HS+C":
            potential_dict[pair] = HardSphereCoulombPotential(
                diameter=params["diameter"],
                epsilon=params["epsilon"],
                q1=params["q1"],
                q2=params["q2"],
                kappa_inv=params["kappa_inv"],
            )
        elif pair_type == "LJ+C":
            potential_dict[pair] = LennardJonesCoulombPotential(
