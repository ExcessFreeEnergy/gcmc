...
class SlitLJPotential(ExtPotential):
    ...
    def __call__(self, position):
        ...
        return np.where(
            (position[0] > self.high) | (position[0] < self.low), very_large_number, energy_low + energy_high
        )
    ...
    def calculate_multiple(self, positions):
        ...
        energies[inside] = np.where(r_low < self.sigma, energy_low, 0.0) + np.where(
            r_high < self.sigma, energy_high, 0.0
        )
...
class TrainingPotential(BaseTrainingPotential):
    ...
    def __call__(self, position):
        ...
        linear_terms = sum(
            self.calculate_linear_potential(Va, Vb, xa, xb, position) for Va, Vb, xa, xb in self.linear_potentials
        )
    ...
    def __call__(self, position):
        ...
        sine_terms = sum(
            ...
            for i, (A, B, phi) in enumerate(
                [
                    (self.A1, self.B1, self.phi1),
                    (self.A2, self.B2, self.phi2),
                    (self.A3, self.B3, self.phi3),
                    (self.A4, self.B4, self.phi4),
                ],
                start=1,
            )
        )
...
class BaseTrainingPotentialWithCharge(ExtPotential):
    def __init__(
        self, q, A1, A2, A3, A4, phi1, phi2, phi3, phi4, q_A1, q_A2, q_A3, q_A4, q_phi1, q_phi2, q_phi3, q_phi4, L
    ):
        self.q = q
        self.A1 = A1
        self.A2 = A2
    ...
    def calculate_q_sines(self, position):
        ...
        sine_terms_q = sum(
            ...
            for i, (A, phi) in enumerate(
                [
                    (self.q_A1, self.q_phi1),
                    (self.q_A2, self.q_phi2),
                    (self.q_A3, self.q_phi3),
                    (self.q_A4, self.q_phi4),
                ],
                start=1,
            )
        )
...
class TrainingPotentialWithChargeCos(BaseTrainingPotentialWithCharge):
    def __init__(
        ...
        super().__init__(
            q, A1, A2, A3, A4, phi1, phi2, phi3, phi4, q_A1, q_A2, q_A3, q_A4, q_phi1, q_phi2, q_phi3, q_phi4, L
        )
        self.linear_potentials = linear_potentials
...
class TrainingPotentialWithChargeCosWithMorseWalls(BaseTrainingPotentialWithCharge):
    ...
    def __call__(self, position):
        ...
        return np.where(
            (position[0] > self.high) | (position[0] < self.low),
            very_large_number,
            energy_low + energy_high + sine_terms + linear_terms + sine_terms_q * self.q + linear_terms_q * self.q,
        )
    ...
    def get_non_electrostatic(self, position):
        ...
        return np.where(
            (position[0] > self.high) | (position[0] < self.low), very_large_number, sine_terms + linear_terms
        )
    ...
    def calculate_multiple(self, positions):
        ...
        energies[inside] = (
            sine_terms + linear_terms + sine_terms_q * self.q + linear_terms_q * self.q + energy_low + energy_high
        )
...
class TrainingPotentialWithChargeCosWithLJ93WallsWithExpField(BaseTrainingPotentialWithCharge):
    ...
    def __call__(self, position):
        ...
        energy_low = (
            self.epsilon_lo * ((2 / 15) * (self.sigma_lo / r_low) ** 9 - (self.sigma_lo / r_low) ** 3) - self.shift_lo
        )
        energy_high = (
            self.epsilon_hi * ((2 / 15) * (self.sigma_hi / r_high) ** 9 - (self.sigma_hi / r_high) ** 3) - self.shift_hi
        )
...
class TrainingPotentialWithWalls(BaseTrainingPotential):
    ...
    def __call__(self, position):
        ...
        return np.where(
            (self.width / 2 > position[0]) | (position[0] > self.L - self.width / 2),
            very_large_number,
            sine_terms + linear_terms,
        )
    ...
    def calculate_multiple(self, positions):
        ...
        energies = np.where(
            (self.width / 2 > positions[:, 0]) | (positions[:, 0] > self.L - self.width / 2),
            very_large_number,
            sine_terms + linear_terms,
        )
        return np.sum(energies)
...
class TrainingPotentialWithLJ93Walls(BaseTrainingPotential):
    ...
    def __call__(self, position):
        ...
        return np.where(
            (position[0] > self.high) | (position[0] < self.low),
            very_large_number,
            energy_low + energy_high + sine_terms + linear_terms,
        )
...
class TrainingPotentialWithChargeWithWalls(BaseTrainingPotentialWithCharge):
    ...
    def __call__(self, position):
        ...
        return np.where(
            (self.width / 2 > position[0]) | (position[0] > self.L - self.width / 2),
            very_large_number,
            sine_terms + linear_terms + sine_terms_q * self.q + linear_terms_q * self.q,
        )
    ...
    def get_electrostatic(self, position):
        ...
        return np.where(
            (self.width / 2 > position[0]) | (position[0] > self.L - self.width / 2),
            0.0,
            sine_terms_q * self.q + linear_terms_q * self.q,
        )
...
class TrainingPotentialWithChargeWithLJ93Walls(BaseTrainingPotentialWithCharge):
    ...
    def get_electrostatic(self, position):
        ...
        return np.where(
            (position[0] > self.high) | (position[0] < self.low), 0.0, sine_terms_q * self.q + linear_terms_q * self.q
        )
        ...
        if ext_type:
            if ext_type == "WallPotential":
                ...
                external_potentials_dict[type] = TrainingPotentialWithChargeWithWalls(
                    ...
                    phi3=params["phi3"],
                    phi4=params["phi4"],
                    q_A1=params["q_A1"] * kB * T,
                    q_A2=params["q_A2"] * kB * T,
                    q_A3=params["q_A3"] * kB * T,
                    q_A4=params["q_A4"] * kB * T,
                    q_phi1=params["q_phi1"],
                    q_phi2=params["q_phi2"],
                    q_phi3=params["q_phi3"],
                    q_phi4=params["q_phi4"],
                    linear_potentials=linear_potentials,
                    q_linear_potentials=q_linear_potentials,
                    L=params["L"],
                    width=params["width"],
                )
                ...
                external_potentials_dict[type] = TrainingPotentialWithChargeCosWithMorseWalls(
                    ...
                    low=params["low"],
                    high=params["high"],
                    D0_lo=params["D0_lo"] * kB * T,
                    r0_lo=params["r0_lo"],
                    a_inv_lo=params["a_inv_lo"],
                    cutoff_lo=params["cutoff_lo"],
                    D0_hi=params["D0_hi"] * kB * T,
                    r0_hi=params["r0_hi"],
                    a_inv_hi=params["a_inv_hi"],
                ...
                external_potentials_dict[type] = TrainingPotentialWithChargeCosWithLJ93WallsWithExpField(
                    ...
                    low=params["low"],
                    high=params["high"],
                    epsilon_lo=params["epsilon_lo"] * kB * T,
                    sigma_lo=params["sigma_lo"],
                    cutoff_lo=params["cutoff_lo"],
                    epsilon_hi=params["epsilon_hi"] * kB * T,
                    sigma_hi=params["sigma_hi"],
                    cutoff_hi=params["cutoff_hi"],
                    phi_0_lo=params["phi_0_lo"] * kB * T,
                    d_lo=params["d_lo"],
                    phi_0_hi=params["phi_0_hi"] * kB * T,
                    d_hi=params["d_hi"],
                )
