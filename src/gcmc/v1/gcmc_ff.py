...
class GCMC_FF_SingleType_Simulation:
    ...
    def write_xyz_header(self):
        ...
        with gzip.open(self.output_xyz + ".gz", "wt"):
            pass
...
class GCMC_FF_TwoType_Simulation:
    ...
    def total_energy(self):
        ...
        if self.number1 > 0 and self.number2 > 0:
            total_pairwise_energy += calculate_pairwise_energy(
                positions_1, positions_2, self.potential_1_2, exclude_self=False
            )

        # Add external potential energy if applicable
        external_energy_1 = (
            np.sum(self.external_potential_1.calculate_multiple(positions_1)) if self.external_potential_1 else 0.0
        )
        external_energy_2 = (
            np.sum(self.external_potential_2.calculate_multiple(positions_2)) if self.external_potential_2 else 0.0
        )
