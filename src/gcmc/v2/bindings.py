...
import ctypes
from ctypes import (
    c_void_p, c_double, c_int, c_bool, c_char_p, c_uint64, c_float,
    Structure, POINTER
)
import numpy as np
...
_lib.gcmc_v2_add_linear_segment.argtypes = [
    c_void_p, c_int, c_double, c_double, c_double, c_double, c_bool
]
...
_lib.gcmc_v2_set_two_type_params.argtypes = [
    c_void_p, c_char_p, c_char_p, c_double, c_double, c_int, c_int
]
...
class CUDAExternalParams(Structure):
    _fields_ = [
        ...
        ("phi1", c_float), ("phi2", c_float), ("phi3", c_float), ("phi4", c_float),
        ("q_A1", c_float), ("q_A2", c_float), ("q_A3", c_float), ("q_A4", c_float),
        ("q_phi1", c_float), ("q_phi2", c_float), ("q_phi3", c_float), ("q_phi4", c_float),
    ]

class CUDABoxConfig(Structure):
    _fields_ = [
        ("mol_type", c_int),
        ("box_x", c_float), ("box_y", c_float), ("box_z", c_float),
        ("beta", c_float),
        ("mu1", c_float), ("mu2", c_float),
        ("bond_length", c_float),
        ("maxdispl", c_float),
        ("prob_insert", c_float), ("prob_delete", c_float),
        ("prob_displace", c_float), ("prob_rotate", c_float), ("prob_mutate", c_float),
        ("global_rc", c_float),
...
try:
    ...
    _lib.run_cuda_batch_gcmc.argtypes = [
        c_int, c_int, c_int,
        POINTER(CUDABoxConfig),
        POINTER(CUDABoxOutput),
        c_uint64
    ]
...
class GCMCSimulationV2:
    ...
    def _configure_potentials(self):
        ...
        for pair_name, p in pair_dict.items():
            ...
            kind = 0
            if ptype == 'LJ': kind = 1
            elif ptype == 'WCA': kind = 2
            elif ptype == 'HS': kind = 3
            elif ptype == 'HS+C': kind = 4
            elif ptype == 'LJ+C': kind = 5
            ...
            _lib.gcmc_v2_set_pair_potential(
                self.handle, s1, s2, kind,
                eps_lj, sig_lj, rc, eps_c, q1, q2, kappa_inv, diameter
            )
            # If H2O, expand H interactions to H1 (site 1) and H2 (site 2)
            if self.config.get('molecule') == 'H2O':
                if s1 == 1 and s2 == 1: # H-H
                    _lib.gcmc_v2_set_pair_potential(self.handle, 2, 2, kind, eps_lj, sig_lj, rc, eps_c, q1, q2, kappa_inv, diameter)
                    _lib.gcmc_v2_set_pair_potential(self.handle, 1, 2, kind, eps_lj, sig_lj, rc, eps_c, q1, q2, kappa_inv, diameter)
                elif s1 == 1 and s2 == 0: # H-O
                    _lib.gcmc_v2_set_pair_potential(self.handle, 2, 0, kind, eps_lj, sig_lj, rc, eps_c, q1, q2, kappa_inv, diameter)
                elif s1 == 0 and s2 == 1: # O-H
                    _lib.gcmc_v2_set_pair_potential(self.handle, 0, 2, kind, eps_lj, sig_lj, rc, eps_c, q1, q2, kappa_inv, diameter)
    ...
    def _configure_external_potentials(self):
        ...
        for name, p in particle_types.items():
            ...
            if name in site_map_abc: site = site_map_abc[name]
            elif name in site_map_h2o: site = site_map_h2o[name]
            elif name in site_map_rpm: site = site_map_rpm[name]
            else: continue

            if vext_type.startswith("TrainingPotentialWithChargeCos"):
                _lib.gcmc_v2_set_external_potential_cos(
                    self.handle, site,
                    float(p.get('low', 2.0)), float(p.get('high', 18.0)), float(p.get('L', 20.0)),
                    float(p.get('epsilon', 1.0)), float(p.get('sigma', 1.0)), float(p.get('cutoff', 0.858)),
                    float(p.get('q', 0.0)),
                    float(p.get('A1', 0.0)), float(p.get('A2', 0.0)), float(p.get('A3', 0.0)), float(p.get('A4', 0.0)),
                    float(p.get('phi1', 0.0)), float(p.get('phi2', 0.0)), float(p.get('phi3', 0.0)), float(p.get('phi4', 0.0)),
                    float(p.get('q_A1', 0.0)), float(p.get('q_A2', 0.0)), float(p.get('q_A3', 0.0)), float(p.get('q_A4', 0.0)),
                    float(p.get('q_phi1', 0.0)), float(p.get('q_phi2', 0.0)), float(p.get('q_phi3', 0.0)), float(p.get('q_phi4', 0.0))
                )
                ...
                for i in range(1, num_segs + 1):
                    ...
                    if va_k in p and vb_k in p:
                        _lib.gcmc_v2_add_linear_segment(
                            self.handle, site,
                            float(p[va_k]), float(p[vb_k]), float(p[xa_k]), float(p[xb_k]),
                            False
                        )
                    ...
                    if q_va_k in p and q_vb_k in p:
                        _lib.gcmc_v2_add_linear_segment(
                            self.handle, site,
                            float(p[q_va_k]), float(p[q_vb_k]), float(p[q_xa_k]), float(p[q_xb_k]),
                            True
                        )
            elif vext_type in ("SlitPotential", "WallPotential"):
                _lib.gcmc_v2_set_external_potential_slit(
                    self.handle, site,
                    float(p.get('low', 2.0)), float(p.get('high', 18.0))
                )
            else:
...
def run_batch_cuda(configs, num_steps=1000, equilibration_steps=200, seed=12345):
    ...
    for b, cfg in enumerate(configs):
        c = c_configs[b]
        mol_flag = cfg.get('molecule', 'None')
        if mol_flag == 'ABC': c.mol_type = 3
        elif mol_flag == 'H2O': c.mol_type = 4
        elif len(cfg.get('particle_types', {})) == 2: c.mol_type = 2
        else: c.mol_type = 1
    ...
    _lib.run_cuda_batch_gcmc(
        num_boxes, num_steps, equilibration_steps,
        c_configs, c_outputs, seed
    )
