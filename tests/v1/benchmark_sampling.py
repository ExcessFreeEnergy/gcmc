...
def benchmark_model(model_name, step_counts=[5000, 10000, 20000]):
    ...
    for n_steps in step_counts:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            print(
                f"Steps: {n_steps:6d} | Particles: {n_particles:3d} | Time: {elapsed:6.2f} s | Rate: {rate:8.1f} steps/s | 1M steps: {time_per_1m:6.2f} min | 1B steps: {time_per_1b:6.2f} hrs"
            )
...
def main():
    ...
    print("=" * 70)
    print(
        f"{'Model':<10} | {'Throughput (steps/s)':<22} | {'Time per 1M steps':<18} | {'Time per 1B steps (Paper)':<25}"
    )
    print("-" * 70)
